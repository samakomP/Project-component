  ...
  <div v-if="isSidebarOpen" class="fixed inset-0 z-50 flex">
    ...
    <div v-if="isAdmin" class="w-[340px] h-full bg-[#f9e5cc] shadow-2xl p-8 flex flex-col items-start">
      ...
      <nav class="flex flex-col gap-12 text-3xl font-serif font-bold w-full pl-6">
        ...
        <RouterLink :to="{ name: 'admin-management', params: { id: authStore.user?.id } }" @click="toggleSidebar" class="text-black hover:opacity-70 hover:underline hover:underline-offset-8 hover:decoration-2 transition cursor-pointer">User</RouterLink>
